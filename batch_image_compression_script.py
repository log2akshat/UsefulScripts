...
def is_valid_logging_status(parser, arg):
    ...
    if not arg in ('on', 'off'):
        parser.error('{} is not a valid input for turning logging on or off! Please specify \
                      \"on\" for turning logging on and \"off\" for turning logging off.'.format(arg))
    return arg


## =========> Command line arguments parsing -- starts <========= ##
PARSER = argparse.ArgumentParser(description='Batch image compression utility. For running this \
                                              program you need to have imagemagick installed on \
                                              your machine.')
